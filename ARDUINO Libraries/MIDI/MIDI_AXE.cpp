...
MIDI_AXE_Class MIDI_AXE;


/*! \brief Default constructor for MIDI_AXE_Class. */
MIDI_AXE_Class::MIDI_AXE_Class()
{ 
...
/*! \brief Default destructor for MIDI_AXE_Class.
...
/*! \brief Call the begin method in the setup() function of the Arduino.
 ...
 - Full thru mirroring
 */
void MIDI_AXE_Class::begin(const byte inChannel)
{
	
	// Initialise the Serial port
	AXE_FX_USE_SERIAL_PORT.begin(MIDI_BAUDRATE);
...
const byte MIDI_AXE_Class::genstatus(const kMIDIType inType,
 ...
 */
void MIDI_AXE_Class::send(kMIDIType type,
					  byte data1,
					  byte data2,
					  byte channel)
{
	...
	if (type <= PitchBend) {
		...
		if (mRunningStatus_TX != statusbyte) {
			...
			AXE_FX_USE_SERIAL_PORT.write(mRunningStatus_TX);
		}
#else
		// Don't care about running status, send the Control byte.
		AXE_FX_USE_SERIAL_PORT.write(statusbyte);
#endif
		
		// Then send data
		AXE_FX_USE_SERIAL_PORT.write(data1);
		if (type != ProgramChange && type != AfterTouchChannel) {
			AXE_FX_USE_SERIAL_PORT.write(data2);
		}
		return;
	}
 ...
 */
void MIDI_AXE_Class::sendNoteOn(byte NoteNumber,
							byte Velocity,
							byte Channel)
{ 
...
/*! \brief Send a Note Off message (a real Note Off, not a Note On with null velocity)
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_AXE_Class::sendNoteOff(byte NoteNumber,
							 byte Velocity,
							 byte Channel)
{
...
/*! \brief Send a Program Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16).
 */
void MIDI_AXE_Class::sendProgramChange(byte ProgramNumber,
								   byte Channel)
{
...
/*! \brief Send a Control Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_AXE_Class::sendControlChange(byte ControlNumber,
								   byte ControlValue,
								   byte Channel)
{
...
/*! \brief Send a Polyphonic AfterTouch message (applies to only one specified note)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_AXE_Class::sendPolyPressure(byte NoteNumber,
								  byte Pressure,
								  byte Channel)
{
...
/*! \brief Send a MonoPhonic AfterTouch message (applies to all notes)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_AXE_Class::sendAfterTouch(byte Pressure,
								byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a signed integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_AXE_Class::sendPitchBend(int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using an unsigned integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_AXE_Class::sendPitchBend(unsigned int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a floating point value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_AXE_Class::sendPitchBend(double PitchValue,
							   byte Channel)
{
 ...
 */
void MIDI_AXE_Class::sendSysEx(int length,
						   ...
						   bool ArrayContainsBoundaries)
{
	...
	if (ArrayContainsBoundaries == false) {
		...
		AXE_FX_USE_SERIAL_PORT.write(0xF0);
		
		for (int i=0;i<length;++i) {
			
			AXE_FX_USE_SERIAL_PORT.write(array[i]);
			
		}
		
		AXE_FX_USE_SERIAL_PORT.write(0xF7);
 ...
 */
void MIDI_AXE_Class::sendTuneRequest()
{
 ...
 */
void MIDI_AXE_Class::sendTimeCodeQuarterFrame(byte TypeNibble, byte ValuesNibble)
{
 ...
 */
void MIDI_AXE_Class::sendTimeCodeQuarterFrame(byte data)
{
	
	AXE_FX_USE_SERIAL_PORT.write((byte)TimeCodeQuarterFrame);
	AXE_FX_USE_SERIAL_PORT.write(data);
 ...
 */
void MIDI_AXE_Class::sendSongPosition(unsigned int Beats)
{
	
	AXE_FX_USE_SERIAL_PORT.write((byte)SongPosition);
	AXE_FX_USE_SERIAL_PORT.write(Beats & 0x7F);
	AXE_FX_USE_SERIAL_PORT.write((Beats >> 7) & 0x7F);
...
void MIDI_AXE_Class::sendSongSelect(byte SongNumber)
{
	
	AXE_FX_USE_SERIAL_PORT.write((byte)SongSelect);
	AXE_FX_USE_SERIAL_PORT.write(SongNumber & 0x7F);
...
/*! \brief Send a Real Time (one byte) message. 
 ...
 @see kMIDIType
 */
void MIDI_AXE_Class::sendRealTime(kMIDIType Type)
{
	switch (Type) {
		...
		case ActiveSensing:
		case SystemReset:
			AXE_FX_USE_SERIAL_PORT.write((byte)Type);
			break;
		default:
...
bool MIDI_AXE_Class::read(const byte inChannel)
{
    
// EF_MODIF
#ifdef DEBUG
    Serial.println();
    Serial.print("MIDI_AXE_FX: START READING "); Serial.println(inChannel, HEX);
#endif  //*/
...
bool MIDI_AXE_Class::parse(byte inChannel)
{ 

	const int bytes_available = AXE_FX_USE_SERIAL_PORT.available();

// EF_MODIF
#ifdef DEBUG
    if(bytes_available > 0 )
    {
       Serial.print("MIDI_AXE_FX: START PARSING ");
       Serial.print(inChannel, DEC); Serial.print(" ");
...
//*/    
	...
	if (bytes_available == 128) {
		...
		const byte extracted = AXE_FX_USE_SERIAL_PORT.read();

// EF_MODIF
#ifdef DEBUG
        Serial.print(extracted, HEX); Serial.print(" "); 
#endif  	
//*/		
...
bool MIDI_AXE_Class::input_filter(byte inChannel)
{
...
void MIDI_AXE_Class::reset_input_attributes()
{
 ...
 */
kMIDIType MIDI_AXE_Class::getType() const
{
 ...
 */
byte MIDI_AXE_Class::getChannel() const
{
...
byte MIDI_AXE_Class::getData1() const
{
...
byte MIDI_AXE_Class::getData2() const
{ 
 ...
 */
const byte * MIDI_AXE_Class::getSysExArray() const
{ 
 ...
 */
unsigned int MIDI_AXE_Class::getSysExArrayLength() const
{
...
bool MIDI_AXE_Class::check() const
{ 
 ...
 */
void MIDI_AXE_Class::setInputChannel(const byte Channel)
{ 
...
void MIDI_AXE_Class::setHandleNoteOff(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOffCallback = fptr; }
void MIDI_AXE_Class::setHandleNoteOn(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOnCallback = fptr; }
void MIDI_AXE_Class::setHandleAfterTouchPoly(void (*fptr)(byte channel, byte note, byte pressure))	{ mAfterTouchPolyCallback = fptr; }
void MIDI_AXE_Class::setHandleControlChange(void (*fptr)(byte channel, byte number, byte value))	{ mControlChangeCallback = fptr; }
void MIDI_AXE_Class::setHandleProgramChange(void (*fptr)(byte channel, byte number))				{ mProgramChangeCallback = fptr; }
void MIDI_AXE_Class::setHandleAfterTouchChannel(void (*fptr)(byte channel, byte pressure))			{ mAfterTouchChannelCallback = fptr; }
void MIDI_AXE_Class::setHandlePitchBend(void (*fptr)(byte channel, int bend))						{ mPitchBendCallback = fptr; }
void MIDI_AXE_Class::setHandleSystemExclusive(void (*fptr)(byte * array, byte size))				{ mSystemExclusiveCallback = fptr; }
void MIDI_AXE_Class::setHandleTimeCodeQuarterFrame(void (*fptr)(byte data))							{ mTimeCodeQuarterFrameCallback = fptr; }
void MIDI_AXE_Class::setHandleSongPosition(void (*fptr)(unsigned int beats))						{ mSongPositionCallback = fptr; }
void MIDI_AXE_Class::setHandleSongSelect(void (*fptr)(byte songnumber))								{ mSongSelectCallback = fptr; }
void MIDI_AXE_Class::setHandleTuneRequest(void (*fptr)(void))										{ mTuneRequestCallback = fptr; }
void MIDI_AXE_Class::setHandleClock(void (*fptr)(void))												{ mClockCallback = fptr; }
void MIDI_AXE_Class::setHandleStart(void (*fptr)(void))												{ mStartCallback = fptr; }
void MIDI_AXE_Class::setHandleContinue(void (*fptr)(void))											{ mContinueCallback = fptr; }
void MIDI_AXE_Class::setHandleStop(void (*fptr)(void))												{ mStopCallback = fptr; }
void MIDI_AXE_Class::setHandleActiveSensing(void (*fptr)(void))										{ mActiveSensingCallback = fptr; }
void MIDI_AXE_Class::setHandleSystemReset(void (*fptr)(void))										{ mSystemResetCallback = fptr; }
 ...
 */
void MIDI_AXE_Class::disconnectCallbackFromType(kMIDIType Type)
{
...
void MIDI_AXE_Class::launchCallback()
{
...
/*! \brief Set the filter for thru mirroring
 ...
 @see kThruFilterMode
 */
void MIDI_AXE_Class::setThruFilterMode(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_AXE_Class::turnThruOn(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_AXE_Class::turnThruOff()
{
...
void MIDI_AXE_Class::thru_filter(byte inChannel)
{
