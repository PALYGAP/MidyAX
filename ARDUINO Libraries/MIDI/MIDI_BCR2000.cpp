...
MIDI_BCR2000_Class MIDI_BCR2000;


/*! \brief Default constructor for MIDI_BCR2000_Class. */
MIDI_BCR2000_Class::MIDI_BCR2000_Class()
{ 
...
/*! \brief Default destructor for MIDI_BCR2000_Class.
...
/*! \brief Call the begin method in the setup() function of the Arduino.
 ...
 - Full thru mirroring
 */
void MIDI_BCR2000_Class::begin(const byte inChannel)
{
	
	// Initialise the Serial port
	BCR2000_USE_SERIAL_PORT.begin(MIDI_BAUDRATE);
...
const byte MIDI_BCR2000_Class::genstatus(const kMIDIType inType,
 ...
 */
void MIDI_BCR2000_Class::send(kMIDIType type,
					  byte data1,
					  byte data2,
					  byte channel)
{
	...
	if (type <= PitchBend) {
		...
		if (mRunningStatus_TX != statusbyte) {
			...
			BCR2000_USE_SERIAL_PORT.write(mRunningStatus_TX);
		}
#else
		// Don't care about running status, send the Control byte.
		BCR2000_USE_SERIAL_PORT.write(statusbyte);
#endif
		
		// Then send data
		BCR2000_USE_SERIAL_PORT.write(data1);
		if (type != ProgramChange && type != AfterTouchChannel) {
			BCR2000_USE_SERIAL_PORT.write(data2);
		}
		return;
	}
 ...
 */
void MIDI_BCR2000_Class::sendNoteOn(byte NoteNumber,
							byte Velocity,
							byte Channel)
{ 
...
/*! \brief Send a Note Off message (a real Note Off, not a Note On with null velocity)
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_BCR2000_Class::sendNoteOff(byte NoteNumber,
							 byte Velocity,
							 byte Channel)
{
...
/*! \brief Send a Program Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16).
 */
void MIDI_BCR2000_Class::sendProgramChange(byte ProgramNumber,
								   byte Channel)
{
...
/*! \brief Send a Control Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_BCR2000_Class::sendControlChange(byte ControlNumber,
								   byte ControlValue,
								   byte Channel)
{
...
/*! \brief Send a Polyphonic AfterTouch message (applies to only one specified note)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_BCR2000_Class::sendPolyPressure(byte NoteNumber,
								  byte Pressure,
								  byte Channel)
{
...
/*! \brief Send a MonoPhonic AfterTouch message (applies to all notes)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_BCR2000_Class::sendAfterTouch(byte Pressure,
								byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a signed integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_BCR2000_Class::sendPitchBend(int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using an unsigned integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_BCR2000_Class::sendPitchBend(unsigned int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a floating point value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_BCR2000_Class::sendPitchBend(double PitchValue,
							   byte Channel)
{
 ...
 */
void MIDI_BCR2000_Class::sendSysEx(int length,
						   ...
						   bool ArrayContainsBoundaries)
{
	...
	if (ArrayContainsBoundaries == false) {
		...
		BCR2000_USE_SERIAL_PORT.write(0xF0);
		
		for (int i=0;i<length;++i) {
			
			BCR2000_USE_SERIAL_PORT.write(array[i]);
			
		}
		
		BCR2000_USE_SERIAL_PORT.write(0xF7);
 ...
 */
void MIDI_BCR2000_Class::sendTuneRequest()
{
 ...
 */
void MIDI_BCR2000_Class::sendTimeCodeQuarterFrame(byte TypeNibble, byte ValuesNibble)
{
 ...
 */
void MIDI_BCR2000_Class::sendTimeCodeQuarterFrame(byte data)
{
	
	BCR2000_USE_SERIAL_PORT.write((byte)TimeCodeQuarterFrame);
	BCR2000_USE_SERIAL_PORT.write(data);
 ...
 */
void MIDI_BCR2000_Class::sendSongPosition(unsigned int Beats)
{
	
	BCR2000_USE_SERIAL_PORT.write((byte)SongPosition);
	BCR2000_USE_SERIAL_PORT.write(Beats & 0x7F);
	BCR2000_USE_SERIAL_PORT.write((Beats >> 7) & 0x7F);
...
void MIDI_BCR2000_Class::sendSongSelect(byte SongNumber)
{
	
	BCR2000_USE_SERIAL_PORT.write((byte)SongSelect);
	BCR2000_USE_SERIAL_PORT.write(SongNumber & 0x7F);
...
/*! \brief Send a Real Time (one byte) message. 
 ...
 @see kMIDIType
 */
void MIDI_BCR2000_Class::sendRealTime(kMIDIType Type)
{
	switch (Type) {
		...
		case ActiveSensing:
		case SystemReset:
			BCR2000_USE_SERIAL_PORT.write((byte)Type);
			break;
		default:
...
bool MIDI_BCR2000_Class::read(const byte inChannel)
{

/*EF_MODIF
#ifdef DEBUG
    Serial.print("BCR2000: START READING");
...
bool MIDI_BCR2000_Class::parse(byte inChannel)
{ 
    
	const int bytes_available = BCR2000_USE_SERIAL_PORT.available();

/* EF_MODIF
#ifdef DEBUG
    //Serial.print("BCR2000: START PARSING");
    //Serial.print("    inChannel:");
    //Serial.print(inChannel, DEC);
...
*/
	...
	if (bytes_available == 128) {
		...
		const byte extracted = BCR2000_USE_SERIAL_PORT.read();
        //byte nextbyte = BCR2000_USE_SERIAL_PORT.peek();
...
bool MIDI_BCR2000_Class::input_filter(byte inChannel)
{
...
void MIDI_BCR2000_Class::reset_input_attributes()
{
 ...
 */
kMIDIType MIDI_BCR2000_Class::getType() const
{
 ...
 */
byte MIDI_BCR2000_Class::getChannel() const
{
...
byte MIDI_BCR2000_Class::getData1() const
{
...
byte MIDI_BCR2000_Class::getData2() const
{ 
 ...
 */
const byte * MIDI_BCR2000_Class::getSysExArray() const
{ 
 ...
 */
unsigned int MIDI_BCR2000_Class::getSysExArrayLength() const
{
...
bool MIDI_BCR2000_Class::check() const
{ 
 ...
 */
void MIDI_BCR2000_Class::setInputChannel(const byte Channel)
{ 
...
void MIDI_BCR2000_Class::setHandleNoteOff(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOffCallback = fptr; }
void MIDI_BCR2000_Class::setHandleNoteOn(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOnCallback = fptr; }
void MIDI_BCR2000_Class::setHandleAfterTouchPoly(void (*fptr)(byte channel, byte note, byte pressure))	{ mAfterTouchPolyCallback = fptr; }
void MIDI_BCR2000_Class::setHandleControlChange(void (*fptr)(byte channel, byte number, byte value))	{ mControlChangeCallback = fptr; }
void MIDI_BCR2000_Class::setHandleProgramChange(void (*fptr)(byte channel, byte number))				{ mProgramChangeCallback = fptr; }
void MIDI_BCR2000_Class::setHandleAfterTouchChannel(void (*fptr)(byte channel, byte pressure))			{ mAfterTouchChannelCallback = fptr; }
void MIDI_BCR2000_Class::setHandlePitchBend(void (*fptr)(byte channel, int bend))						{ mPitchBendCallback = fptr; }
void MIDI_BCR2000_Class::setHandleSystemExclusive(void (*fptr)(byte * array, byte size))				{ mSystemExclusiveCallback = fptr; }
void MIDI_BCR2000_Class::setHandleTimeCodeQuarterFrame(void (*fptr)(byte data))							{ mTimeCodeQuarterFrameCallback = fptr; }
void MIDI_BCR2000_Class::setHandleSongPosition(void (*fptr)(unsigned int beats))						{ mSongPositionCallback = fptr; }
void MIDI_BCR2000_Class::setHandleSongSelect(void (*fptr)(byte songnumber))								{ mSongSelectCallback = fptr; }
void MIDI_BCR2000_Class::setHandleTuneRequest(void (*fptr)(void))										{ mTuneRequestCallback = fptr; }
void MIDI_BCR2000_Class::setHandleClock(void (*fptr)(void))												{ mClockCallback = fptr; }
void MIDI_BCR2000_Class::setHandleStart(void (*fptr)(void))												{ mStartCallback = fptr; }
void MIDI_BCR2000_Class::setHandleContinue(void (*fptr)(void))											{ mContinueCallback = fptr; }
void MIDI_BCR2000_Class::setHandleStop(void (*fptr)(void))												{ mStopCallback = fptr; }
void MIDI_BCR2000_Class::setHandleActiveSensing(void (*fptr)(void))										{ mActiveSensingCallback = fptr; }
void MIDI_BCR2000_Class::setHandleSystemReset(void (*fptr)(void))										{ mSystemResetCallback = fptr; }
 ...
 */
void MIDI_BCR2000_Class::disconnectCallbackFromType(kMIDIType Type)
{
...
void MIDI_BCR2000_Class::launchCallback()
{
...
/*! \brief Set the filter for thru mirroring
 ...
 @see kThruFilterMode
 */
void MIDI_BCR2000_Class::setThruFilterMode(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_BCR2000_Class::turnThruOn(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_BCR2000_Class::turnThruOff()
{
...
void MIDI_BCR2000_Class::thru_filter(byte inChannel)
{
