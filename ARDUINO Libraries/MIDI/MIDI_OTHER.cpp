...
MIDI_OTHER_Class MIDI_OTHER;


/*! \brief Default constructor for MIDI_OTHER_Class. */
MIDI_OTHER_Class::MIDI_OTHER_Class()
{ 
...
/*! \brief Default destructor for MIDI_OTHER_Class.
...
/*! \brief Call the begin method in the setup() function of the Arduino.
 ...
 - Full thru mirroring
 */
void MIDI_OTHER_Class::begin(const byte inChannel)
{
	
	// Initialise the Serial port
	OTHER_USE_SERIAL_PORT.begin(MIDI_BAUDRATE);
...
const byte MIDI_OTHER_Class::genstatus(const kMIDIType inType,
 ...
 */
void MIDI_OTHER_Class::send(kMIDIType type,
					  byte data1,
					  byte data2,
					  byte channel)
{
	...
	if (type <= PitchBend) {
		...
		if (mRunningStatus_TX != statusbyte) {
			...
			OTHER_USE_SERIAL_PORT.write(mRunningStatus_TX);
		}
#else
		// Don't care about running status, send the Control byte.
		OTHER_USE_SERIAL_PORT.write(statusbyte);
#endif
		
		// Then send data
		OTHER_USE_SERIAL_PORT.write(data1);
		if (type != ProgramChange && type != AfterTouchChannel) {
			OTHER_USE_SERIAL_PORT.write(data2);
		}
		return;
	}
 ...
 */
void MIDI_OTHER_Class::sendNoteOn(byte NoteNumber,
							byte Velocity,
							byte Channel)
{ 
...
/*! \brief Send a Note Off message (a real Note Off, not a Note On with null velocity)
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_OTHER_Class::sendNoteOff(byte NoteNumber,
							 byte Velocity,
							 byte Channel)
{
...
/*! \brief Send a Program Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16).
 */
void MIDI_OTHER_Class::sendProgramChange(byte ProgramNumber,
								   byte Channel)
{
...
/*! \brief Send a Control Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_OTHER_Class::sendControlChange(byte ControlNumber,
								   byte ControlValue,
								   byte Channel)
{
...
/*! \brief Send a Polyphonic AfterTouch message (applies to only one specified note)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_OTHER_Class::sendPolyPressure(byte NoteNumber,
								  byte Pressure,
								  byte Channel)
{
...
/*! \brief Send a MonoPhonic AfterTouch message (applies to all notes)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_OTHER_Class::sendAfterTouch(byte Pressure,
								byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a signed integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_OTHER_Class::sendPitchBend(int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using an unsigned integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_OTHER_Class::sendPitchBend(unsigned int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a floating point value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_OTHER_Class::sendPitchBend(double PitchValue,
							   byte Channel)
{
 ...
 */
void MIDI_OTHER_Class::sendSysEx(int length,
						   ...
						   bool ArrayContainsBoundaries)
{
	...
	if (ArrayContainsBoundaries == false) {
		...
		OTHER_USE_SERIAL_PORT.write(0xF0);
		
		for (int i=0;i<length;++i) {
			
			OTHER_USE_SERIAL_PORT.write(array[i]);
			
		}
		
		OTHER_USE_SERIAL_PORT.write(0xF7);
 ...
 */
void MIDI_OTHER_Class::sendTuneRequest()
{
 ...
 */
void MIDI_OTHER_Class::sendTimeCodeQuarterFrame(byte TypeNibble, byte ValuesNibble)
{
 ...
 */
void MIDI_OTHER_Class::sendTimeCodeQuarterFrame(byte data)
{
	
	OTHER_USE_SERIAL_PORT.write((byte)TimeCodeQuarterFrame);
	OTHER_USE_SERIAL_PORT.write(data);
 ...
 */
void MIDI_OTHER_Class::sendSongPosition(unsigned int Beats)
{
	
	OTHER_USE_SERIAL_PORT.write((byte)SongPosition);
	OTHER_USE_SERIAL_PORT.write(Beats & 0x7F);
	OTHER_USE_SERIAL_PORT.write((Beats >> 7) & 0x7F);
...
void MIDI_OTHER_Class::sendSongSelect(byte SongNumber)
{
	
	OTHER_USE_SERIAL_PORT.write((byte)SongSelect);
	OTHER_USE_SERIAL_PORT.write(SongNumber & 0x7F);
...
/*! \brief Send a Real Time (one byte) message. 
 ...
 @see kMIDIType
 */
void MIDI_OTHER_Class::sendRealTime(kMIDIType Type)
{
	switch (Type) {
		...
		case ActiveSensing:
		case SystemReset:
			OTHER_USE_SERIAL_PORT.write((byte)Type);
			break;
		default:
...
bool MIDI_OTHER_Class::read(const byte inChannel)
{

/*EF_MODIF
#ifdef DEBUG
    Serial.print("OTHER: START READING");
...
bool MIDI_OTHER_Class::parse(byte inChannel)
{ 
    
	const int bytes_available = OTHER_USE_SERIAL_PORT.available();

/* EF_MODIF
#ifdef DEBUG
    //Serial.print("OTHER: START PARSING");
    //Serial.print("    inChannel:");
    //Serial.print(inChannel, DEC);
...
*/
	...
	if (bytes_available == 128) {
		...
		const byte extracted = OTHER_USE_SERIAL_PORT.read();
        //byte nextbyte = OTHER_USE_SERIAL_PORT.peek();
...
bool MIDI_OTHER_Class::input_filter(byte inChannel)
{
...
void MIDI_OTHER_Class::reset_input_attributes()
{
 ...
 */
kMIDIType MIDI_OTHER_Class::getType() const
{
 ...
 */
byte MIDI_OTHER_Class::getChannel() const
{
...
byte MIDI_OTHER_Class::getData1() const
{
...
byte MIDI_OTHER_Class::getData2() const
{ 
 ...
 */
const byte * MIDI_OTHER_Class::getSysExArray() const
{ 
 ...
 */
unsigned int MIDI_OTHER_Class::getSysExArrayLength() const
{
...
bool MIDI_OTHER_Class::check() const
{ 
 ...
 */
void MIDI_OTHER_Class::setInputChannel(const byte Channel)
{ 
...
void MIDI_OTHER_Class::setHandleNoteOff(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOffCallback = fptr; }
void MIDI_OTHER_Class::setHandleNoteOn(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOnCallback = fptr; }
void MIDI_OTHER_Class::setHandleAfterTouchPoly(void (*fptr)(byte channel, byte note, byte pressure))	{ mAfterTouchPolyCallback = fptr; }
void MIDI_OTHER_Class::setHandleControlChange(void (*fptr)(byte channel, byte number, byte value))	{ mControlChangeCallback = fptr; }
void MIDI_OTHER_Class::setHandleProgramChange(void (*fptr)(byte channel, byte number))				{ mProgramChangeCallback = fptr; }
void MIDI_OTHER_Class::setHandleAfterTouchChannel(void (*fptr)(byte channel, byte pressure))			{ mAfterTouchChannelCallback = fptr; }
void MIDI_OTHER_Class::setHandlePitchBend(void (*fptr)(byte channel, int bend))						{ mPitchBendCallback = fptr; }
void MIDI_OTHER_Class::setHandleSystemExclusive(void (*fptr)(byte * array, byte size))				{ mSystemExclusiveCallback = fptr; }
void MIDI_OTHER_Class::setHandleTimeCodeQuarterFrame(void (*fptr)(byte data))							{ mTimeCodeQuarterFrameCallback = fptr; }
void MIDI_OTHER_Class::setHandleSongPosition(void (*fptr)(unsigned int beats))						{ mSongPositionCallback = fptr; }
void MIDI_OTHER_Class::setHandleSongSelect(void (*fptr)(byte songnumber))								{ mSongSelectCallback = fptr; }
void MIDI_OTHER_Class::setHandleTuneRequest(void (*fptr)(void))										{ mTuneRequestCallback = fptr; }
void MIDI_OTHER_Class::setHandleClock(void (*fptr)(void))												{ mClockCallback = fptr; }
void MIDI_OTHER_Class::setHandleStart(void (*fptr)(void))												{ mStartCallback = fptr; }
void MIDI_OTHER_Class::setHandleContinue(void (*fptr)(void))											{ mContinueCallback = fptr; }
void MIDI_OTHER_Class::setHandleStop(void (*fptr)(void))												{ mStopCallback = fptr; }
void MIDI_OTHER_Class::setHandleActiveSensing(void (*fptr)(void))										{ mActiveSensingCallback = fptr; }
void MIDI_OTHER_Class::setHandleSystemReset(void (*fptr)(void))										{ mSystemResetCallback = fptr; }
 ...
 */
void MIDI_OTHER_Class::disconnectCallbackFromType(kMIDIType Type)
{
...
void MIDI_OTHER_Class::launchCallback()
{
...
/*! \brief Set the filter for thru mirroring
 ...
 @see kThruFilterMode
 */
void MIDI_OTHER_Class::setThruFilterMode(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_OTHER_Class::turnThruOn(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_OTHER_Class::turnThruOff()
{
...
void MIDI_OTHER_Class::thru_filter(byte inChannel)
{
