...
MIDI_FOOTPEDAL_Class MIDI_FOOTPEDAL;


/*! \brief Default constructor for MIDI_FOOTPEDAL_Class. */
MIDI_FOOTPEDAL_Class::MIDI_FOOTPEDAL_Class()
{ 
...
/*! \brief Default destructor for MIDI_FOOTPEDAL_Class.
...
/*! \brief Call the begin method in the setup() function of the Arduino.
 ...
 - Full thru mirroring
 */
void MIDI_FOOTPEDAL_Class::begin(const byte inChannel)
{
	
	// Initialise the Serial port
	FOOTPEDAL_USE_SERIAL_PORT.begin(MIDI_BAUDRATE);
...
const byte MIDI_FOOTPEDAL_Class::genstatus(const kMIDIType inType,
 ...
 */
void MIDI_FOOTPEDAL_Class::send(kMIDIType type,
					  byte data1,
					  byte data2,
					  byte channel)
{
	...
	if (type <= PitchBend) {
		...
		if (mRunningStatus_TX != statusbyte) {
			...
			FOOTPEDAL_USE_SERIAL_PORT.write(mRunningStatus_TX);
		}
#else
		// Don't care about running status, send the Control byte.
		FOOTPEDAL_USE_SERIAL_PORT.write(statusbyte);
#endif
		
		// Then send data
		FOOTPEDAL_USE_SERIAL_PORT.write(data1);
		if (type != ProgramChange && type != AfterTouchChannel) {
			FOOTPEDAL_USE_SERIAL_PORT.write(data2);
		}
		return;
	}
 ...
 */
void MIDI_FOOTPEDAL_Class::sendNoteOn(byte NoteNumber,
							byte Velocity,
							byte Channel)
{ 
...
/*! \brief Send a Note Off message (a real Note Off, not a Note On with null velocity)
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_FOOTPEDAL_Class::sendNoteOff(byte NoteNumber,
							 byte Velocity,
							 byte Channel)
{
...
/*! \brief Send a Program Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16).
 */
void MIDI_FOOTPEDAL_Class::sendProgramChange(byte ProgramNumber,
								   byte Channel)
{
...
/*! \brief Send a Control Change message 
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_FOOTPEDAL_Class::sendControlChange(byte ControlNumber,
								   byte ControlValue,
								   byte Channel)
{
...
/*! \brief Send a Polyphonic AfterTouch message (applies to only one specified note)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_FOOTPEDAL_Class::sendPolyPressure(byte NoteNumber,
								  byte Pressure,
								  byte Channel)
{
...
/*! \brief Send a MonoPhonic AfterTouch message (applies to all notes)
 ...
 \param Channel			The channel on which the message will be sent (1 to 16). 
 */
void MIDI_FOOTPEDAL_Class::sendAfterTouch(byte Pressure,
								byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a signed integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_FOOTPEDAL_Class::sendPitchBend(int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using an unsigned integer value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_FOOTPEDAL_Class::sendPitchBend(unsigned int PitchValue,
							   byte Channel)
{
...
/*! \brief Send a Pitch Bend message using a floating point value.
 ...
 \param Channel		The channel on which the message will be sent (1 to 16).
 */
void MIDI_FOOTPEDAL_Class::sendPitchBend(double PitchValue,
							   byte Channel)
{
 ...
 */
void MIDI_FOOTPEDAL_Class::sendSysEx(int length,
						   ...
						   bool ArrayContainsBoundaries)
{
	...
	if (ArrayContainsBoundaries == false) {
		...
		FOOTPEDAL_USE_SERIAL_PORT.write(0xF0);
		
		for (int i=0;i<length;++i) {
			
			FOOTPEDAL_USE_SERIAL_PORT.write(array[i]);
			
		}
		
		FOOTPEDAL_USE_SERIAL_PORT.write(0xF7);
 ...
 */
void MIDI_FOOTPEDAL_Class::sendTuneRequest()
{
 ...
 */
void MIDI_FOOTPEDAL_Class::sendTimeCodeQuarterFrame(byte TypeNibble, byte ValuesNibble)
{
 ...
 */
void MIDI_FOOTPEDAL_Class::sendTimeCodeQuarterFrame(byte data)
{
	
	FOOTPEDAL_USE_SERIAL_PORT.write((byte)TimeCodeQuarterFrame);
	FOOTPEDAL_USE_SERIAL_PORT.write(data);
 ...
 */
void MIDI_FOOTPEDAL_Class::sendSongPosition(unsigned int Beats)
{
	
	FOOTPEDAL_USE_SERIAL_PORT.write((byte)SongPosition);
	FOOTPEDAL_USE_SERIAL_PORT.write(Beats & 0x7F);
	FOOTPEDAL_USE_SERIAL_PORT.write((Beats >> 7) & 0x7F);
...
void MIDI_FOOTPEDAL_Class::sendSongSelect(byte SongNumber)
{
	
	FOOTPEDAL_USE_SERIAL_PORT.write((byte)SongSelect);
	FOOTPEDAL_USE_SERIAL_PORT.write(SongNumber & 0x7F);
...
/*! \brief Send a Real Time (one byte) message. 
 ...
 @see kMIDIType
 */
void MIDI_FOOTPEDAL_Class::sendRealTime(kMIDIType Type)
{
	switch (Type) {
		...
		case ActiveSensing:
		case SystemReset:
			FOOTPEDAL_USE_SERIAL_PORT.write((byte)Type);
			break;
		default:
 ...
 */
bool MIDI_FOOTPEDAL_Class::read()
{
...
bool MIDI_FOOTPEDAL_Class::read(const byte inChannel)
{

/*EF_MODIF
#ifdef DEBUG
    Serial.print("FOOTPEDAL: START READING");
...
*/
	...
	if (parse(inChannel)) {
        ...
        /* EF_MODIF
         #ifdef DEBUG
         Serial.print("GOT TO input_filter  with mMessage.type = ");
         Serial.print(mMessage.type, HEX);
         Serial.print("   mMessage.data1 = ");
         Serial.print(mMessage.data1, HEX);
         Serial.print("   mMessage.data2 = ");
         Serial.println(mMessage.data2, HEX);
         #endif			
         //*/
...
/* EF_MODIF
#ifdef DEBUG
            Serial.print("GOT TO launchCallback  with mMessage.type = ");
            Serial.print(mMessage.type, HEX);
            Serial.print("   mMessage.data1 = ");
            Serial.print(mMessage.data1, HEX);
            Serial.print("   mMessage.data2 = ");
...
bool MIDI_FOOTPEDAL_Class::parse(byte inChannel)
{ 
    
	const int bytes_available = FOOTPEDAL_USE_SERIAL_PORT.available();

/* EF_MODIF
#ifdef DEBUG
    //Serial.print("FOOTPEDAL: START PARSING");
    //Serial.print("    inChannel:");
    //Serial.print(inChannel, DEC);
...
*/
	...
	if (bytes_available == 128) {
		FOOTPEDAL_USE_SERIAL_PORT.flush();
	}	
	else {
		...
		const byte extracted = FOOTPEDAL_USE_SERIAL_PORT.read();
        //byte nextbyte = FOOTPEDAL_USE_SERIAL_PORT.peek();

/* EF_MODIF
#ifdef DEBUG
        Serial.print("    extracted:");
        Serial.println(extracted, HEX);
...
/*#ifdef DEBUG
            Serial.print("    mRunningStatus_RX:");
            Serial.print(mRunningStatus_RX);
            Serial.println();
#endif*/
...
bool MIDI_FOOTPEDAL_Class::input_filter(byte inChannel)
{
...
void MIDI_FOOTPEDAL_Class::reset_input_attributes()
{
 ...
 */
kMIDIType MIDI_FOOTPEDAL_Class::getType() const
{
 ...
 */
byte MIDI_FOOTPEDAL_Class::getChannel() const
{
...
byte MIDI_FOOTPEDAL_Class::getData1() const
{
...
byte MIDI_FOOTPEDAL_Class::getData2() const
{ 
 ...
 */
const byte * MIDI_FOOTPEDAL_Class::getSysExArray() const
{ 
 ...
 */
unsigned int MIDI_FOOTPEDAL_Class::getSysExArrayLength() const
{
...
bool MIDI_FOOTPEDAL_Class::check() const
{ 
 ...
 */
void MIDI_FOOTPEDAL_Class::setInputChannel(const byte Channel)
{ 
...
void MIDI_FOOTPEDAL_Class::setHandleNoteOff(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOffCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleNoteOn(void (*fptr)(byte channel, byte note, byte velocity))			{ mNoteOnCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleAfterTouchPoly(void (*fptr)(byte channel, byte note, byte pressure))	{ mAfterTouchPolyCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleControlChange(void (*fptr)(byte channel, byte number, byte value))	{ mControlChangeCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleProgramChange(void (*fptr)(byte channel, byte number))				{ mProgramChangeCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleAfterTouchChannel(void (*fptr)(byte channel, byte pressure))			{ mAfterTouchChannelCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandlePitchBend(void (*fptr)(byte channel, int bend))						{ mPitchBendCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleSystemExclusive(void (*fptr)(byte * array, byte size))				{ mSystemExclusiveCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleTimeCodeQuarterFrame(void (*fptr)(byte data))							{ mTimeCodeQuarterFrameCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleSongPosition(void (*fptr)(unsigned int beats))						{ mSongPositionCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleSongSelect(void (*fptr)(byte songnumber))								{ mSongSelectCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleTuneRequest(void (*fptr)(void))										{ mTuneRequestCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleClock(void (*fptr)(void))												{ mClockCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleStart(void (*fptr)(void))												{ mStartCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleContinue(void (*fptr)(void))											{ mContinueCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleStop(void (*fptr)(void))												{ mStopCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleActiveSensing(void (*fptr)(void))										{ mActiveSensingCallback = fptr; }
void MIDI_FOOTPEDAL_Class::setHandleSystemReset(void (*fptr)(void))										{ mSystemResetCallback = fptr; }
 ...
 */
void MIDI_FOOTPEDAL_Class::disconnectCallbackFromType(kMIDIType Type)
{
...
void MIDI_FOOTPEDAL_Class::launchCallback()
{
...
/*! \brief Set the filter for thru mirroring
 ...
 @see kThruFilterMode
 */
void MIDI_FOOTPEDAL_Class::setThruFilterMode(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_FOOTPEDAL_Class::turnThruOn(kThruFilterMode inThruFilterMode)
{ 
...
void MIDI_FOOTPEDAL_Class::turnThruOff()
{
...
void MIDI_FOOTPEDAL_Class::thru_filter(byte inChannel)
{
